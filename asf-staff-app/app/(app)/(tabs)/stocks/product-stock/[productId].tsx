...
import { C } from "../_lib/stockTokens";

// ─── Types ────────────────────────────────────────────────────────────────────
type StockEntry = ReturnType<typeof useProductStockContext>["productStocks"][number];

// ─── Entry Card ───────────────────────────────────────────────────────────────
function StockEntryCard({
  item,
  index,
}: Readonly<{ item: StockEntry; index: number }>): React.ReactElement {
      ...
      >
        <Text
          style={{
            fontSize: 12,
            fontWeight: "600",
            color: isActive ? "#059669" : C.danger,
          }}
        >
          {isActive ? "有货" : "无货"}
        </Text>
...
// ─── Entries Section ──────────────────────────────────────────────────────────
interface StockEntriesSectionProps {
  stocks: ReadonlyArray<StockEntry>;
  onAddFirst: () => void;
}

function StockEntriesSection({
  stocks,
  onAddFirst,
}: Readonly<StockEntriesSectionProps>): React.ReactElement {
  if (stocks.length === 0) {
    return (
  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <StockEntryCard item={item} index={index} />
      )}
    />
  );
}

/** Stock detail screen living inside the Stocks tab stack so back navigation stays in Stocks. */
export default function StocksProductStockScreen(): React.ReactElement {
  const { productId } = useLocalSearchParams<{ productId: string }>();
  const router = useRouter();
  const { products } = useProductContext();
  const { productStocks, createProductStock, updateProductStock, loading } =
  ...
  const stockStatusColor = (() => {
    if (totalStock === 0) return C.danger;
    if (totalStock < 5) return "#D97706";
    return "#22C55E";
  })();

  const stockStatusLabel = (() => {
    if (totalStock === 0) return "缺货";
    if (totalStock < 5) return "库存不足";
    return "库存充足";
  })();
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
        ...
        >
          <View>
            ...
            <Text style={{ fontSize: 40, fontWeight: "800", color: stockStatusColor }}>
              {totalStock}
            </Text>
        ...
        {loading ? (
          ...
          <StockEntriesSection
            stocks={stocks}
            onAddFirst={() => { setQtyText(""); setAddModalOpen(true); }}
          />
        )}
      </ScrollView>
          ...
          >
            ...
            >
              ...
              >
                ...
                <Pressable onPress={() => setAdjustModalOpen(false)}>
                  <Ionicons name="close" size={20} color={C.muted} />
                </Pressable>
              </View>
              <Text style={{ fontSize: 13, color: C.muted, marginBottom: 16 }}>
                {`当前库存总量：${totalStock} 件。请输入新的总数量。`}
              </Text>
              <Text
                style={{
                  fontSize: 12,
