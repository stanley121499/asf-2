...
/** AsyncStorage key for dismissed announcement IDs (JSON string array). */
const DISMISSED_KEY = "asf-dismissed-announcements";

/**
 * Parses dismissed IDs from persisted JSON.
 */
...
/**
 * Persists dismissed IDs to AsyncStorage.
 */
async function persistDismissedIds(ids: Set<string>): Promise<void> {
  try {
    ...
    // Ignore quota / storage errors on device
  }
}
...
export interface AnnouncementContextProps {
  ...
  loading: boolean;
  /** Dismisses the announcement with the given ID for this device. */
...
export function AnnouncementProvider({ children }: PropsWithChildren): React.ReactElement {
  ...
  const [dismissLoaded, setDismissLoaded] = useState<boolean>(false);
  ...
  }, [dismissLoaded, fetchAnnouncement]);
