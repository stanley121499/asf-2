---
title: "Source: Performance Fix Agent Prompts — Round 4 (2026)"
type: source
updated: 2026-04-09
tags: [ingest, asf-2, raw-doc]
raw: "raw/sources/docs/PERFORMANCE_FIX_AGENT_PROMPTS_2026_ROUND4.md"
---

# Source: Performance Fix Agent Prompts — Round 4 (2026)

**Raw:** [PERFORMANCE_FIX_AGENT_PROMPTS_2026_ROUND4.md](../../raw/sources/docs/PERFORMANCE_FIX_AGENT_PROMPTS_2026_ROUND4.md)

## Summary

> **For the implementer:** Each section below is a self-contained prompt for a separate agent run. Copy the full section content when passing to the agent. Each agent works on a distinct set of files with no overlap. Do NOT redo Round 3 fixes that are confirmed applied (e.g., `home.tsx`, `HomeHighlightsCard.tsx`, `o...

## Document outline (first headings)

- AGENT 1 — `Math.random()` Upload IDs + `Array(1).fill()` Anti-Pattern
- AGENT 2 — Round 3 Residue: Chat, Internal Chat, CheckoutButton, Stock & Post Modals
- AGENT 3 — Settings, Cart, Payments, Orders: console Cleanup
- AGENT 4 — Schedule, Deleted Products, Support, Landing Pages: console.error Cleanup

## Key claims / scope

- This page is the **per-source** wiki entry for one mirrored doc; the authoritative text is always the raw file.
- For cross-doc themes (performance rounds, redesign prompts), use [[wiki/sources/batch-2026-04-09-docs-and-root-readme]] and [[wiki/entities/asf-2]].

## Related

- [[wiki/entities/asf-2]]
- [[wiki/sources/batch-2026-04-09-docs-and-root-readme]] — corpus overview
- Related theme: performance / media (see also raw PERFORMANCE_ISSUES.md)

## Open questions

- _(Fill after deep read or when using this doc to drive work.)_

