...
export const PointsMembershipProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  const api = useMemo<PointsMembershipAPI>(() => {
    return {
      // user_points
      async getUserPointsById(id: string): Promise<UserPointsRow | null> {
      ...
      async getUserPointsByUserId(userId: string): Promise<UserPointsRow | null> {
      ...
      async listUserPoints(limit?: number): Promise<UserPointsRow[]> {
      ...
      async createUserPoints(payload: UserPointsInsert): Promise<UserPointsRow> {
      ...
      async updateUserPoints(id: string, payload: UserPointsUpdate): Promise<UserPointsRow> {
        if (!isNonEmptyString(id)) {
          throw new Error("Invalid id: must be a non-empty string")
        }
        if (payload.user_id !== undefined && payload.user_id !== null && !isNonEmptyString(payload.user_id)) {
      ...
      async deleteUserPoints(id: string): Promise<void> {
        if (!isNonEmptyString(id)) {
          throw new Error("Invalid id: must be a non-empty string")
        }
      ...
      // user_points_logs
      async getUserPointsLogById(id: number): Promise<UserPointsLogRow | null> {
        if (!isNumber(id)) {
      ...
      async listUserPointsLogsByPointId(pointId: string, limit?: number): Promise<UserPointsLogRow[]> {
      ...
      async createUserPointsLog(payload: UserPointsLogInsert): Promise<UserPointsLogRow> {
      ...
      async updateUserPointsLog(id: number, payload: UserPointsLogUpdate): Promise<UserPointsLogRow> {
        if (!isNumber(id)) {
          throw new Error("Invalid id: must be a finite number")
        }
        if (payload.point_id !== undefined && payload.point_id !== null && !isNonEmptyString(payload.point_id)) {
      ...
      async deleteUserPointsLog(id: number): Promise<void> {
        if (!isNumber(id)) {
          throw new Error("Invalid id: must be a finite number")
        }
      ...
      // membership_tiers
      async getMembershipTierById(id: string): Promise<MembershipTierRow | null> {
        if (!isNonEmptyString(id)) {
          throw new Error("Invalid id: must be a non-empty string")
        }
      ...
      async listMembershipTiers(onlyActive?: boolean): Promise<MembershipTierRow[]> {
      ...
      async createMembershipTier(payload: MembershipTierInsert): Promise<MembershipTierRow> {
      ...
      async updateMembershipTier(id: string, payload: MembershipTierUpdate): Promise<MembershipTierRow> {
        if (!isNonEmptyString(id)) {
          throw new Error("Invalid id: must be a non-empty string")
        }
        if (payload.name !== undefined && payload.name !== null && !isNonEmptyString(payload.name)) {
