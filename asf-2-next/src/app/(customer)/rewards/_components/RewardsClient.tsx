...
/** Number of stamp slots in `user_stamps.stamps` (must match DB constraint). */
const STAMP_SLOT_COUNT = 9;

/**
 * Builds a fresh boolean array for stamp UI state.
 */
...
/**
 * Validates and normalizes a `stamps` array loaded from Supabase.
 *
 * @param value - Raw JSON/DB value.
 * @returns Nine booleans, or null when the payload is unusable.
 */
function normalizeStamps(value: unknown): boolean[] | null {
  if (!Array.isArray(value) || value.length !== STAMP_SLOT_COUNT) {
    return null;
  }
...
/**
 * Loads the signed-in user's stamp row, inserting a default row if missing.
 *
 * @param userId - `auth.users` id (matches `user_stamps.user_id`).
 * @returns Normalized stamp flags.
 */
async function loadOrCreateUserStamps(userId: string): Promise<boolean[]> {
  ...
  const parsedExisting = existing ? normalizeStamps(existing.stamps) : null;
  if (parsedExisting) {
  ...
  const { data: row, error: againError } = await supabase
    .from("user_stamps")
    .select("stamps")
    .eq("user_id", userId)
    .single();
  ...
  const parsed = normalizeStamps(row.stamps);
  return parsed ?? createEmptyStamps();
}
...
const RewardsClient: React.FC = () => {
  ...
  useEffect(() => {
    /**
     * Wait until auth has finished resolving so we do not treat a briefly-null
     * `user` as "signed out" and reset the stamp grid before the session loads.
     */
    if (authLoading) {
      return;
    }
    ...
    const run = async (): Promise<void> => {
      ...
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          console.error("user_stamps load failed:", err instanceof Error ? err.message : err);
        }
        if (!cancelled) {
          setStamps(createEmptyStamps());
        }
  ...
  const handleStampClick = useCallback(
    async (index: number) => {
      ...
      if (error) {
        setStamps(previous);
        if (process.env.NODE_ENV === "development") {
          console.error("user_stamps update failed:", error.message);
        }
      }
    },
  ...
  const remainingStamps = STAMP_SLOT_COUNT - completedStamps;
    ...
    <div className="min-h-screen bg-[var(--color-bg)] pb-24">
      ...
      <div className="card-panel mx-4 mt-6 p-5">
          ...
          {stamps.map((isStamped, idx) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (isNext) {
                    void handleStampClick(idx);
                  }
                }}
                disabled={stampsLoading || (!isNext && !isStamped)}
                className={`
        ...
        <div className="text-center text-sm font-medium text-[var(--color-text)] bg-gray-50 py-3 rounded-xl border border-gray-100">
          已集 {completedStamps}/{STAMP_SLOT_COUNT} 枚 · <span className="text-[var(--color-accent)]">{remainingStamps > 0 ? `${remainingStamps} 枚可获奖励` : "已获得所有奖励！"}</span>
        </div>
      </div>
      ...
      <div className="card-panel mx-4 mt-6 p-5">
        ...
        <button type="button" className="w-full text-center text-xs text-[var(--color-muted)] mt-4 pt-4 border-t border-gray-50">
          查看全部
        </button>
      </div>
