...
/**
 * Loads and mutates store locations via Next.js API routes (service role on the server).
 */
export function StoreLocationProvider({
  ...
  const refreshStoreLocations = useCallback(async (): Promise<void> => {
    setLoading(true);
    try {
      const res = await fetch("/api/store-locations", { method: "GET" });
  ...
  const value = useMemo<StoreLocationContextValue>(
    () => ({
      storeLocations,
      loading,
      refreshStoreLocations,
...
/**
 * Access store location state and mutations.
 */
