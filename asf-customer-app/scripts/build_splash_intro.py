#!/usr/bin/env python3
"""Bundle letter-cascade splash HTML for in-app WebView (no external deps)."""
...
THEME_JS = ROOT / "assets" / "splash" / "pixel2motion-output" / "theme-bridge.js"
OUT_HTML = ROOT / "assets" / "splash" / "intro" / "splash-intro.html"

MOBILE_CSS = """
/* Mobile embed — logo only, no demo chrome */
/* Forced light surface for the in-app splash (overrides dark :root default). */
html, body {
  background: #ffffff;
}
...
def main() -> int:
    if not SRC_HTML.exists():
        raise SystemExit(f"Missing source HTML: {SRC_HTML}")
    ...
    # Strip theme bar mount for mobile (RN passes ?theme=)
    html = html.replace(
        "mountThemeBar();",
